    ...
    if (!commentsLoading) {
        return (
            <section className='comments-container'>
                <h3>Comments</h3>
                <CommentAdd review_id={review_id} setComments={setComments}/>
                ...
                {comments.length > 0 && comments.map(({comment_id, author, body, created_at, votes}) => {
                    return <SingleComment key={comment_id} comment_id={comment_id} author={author} body={body} created_at={created_at} votes={votes}/>
                })}
            </section>
        );
    } else {
